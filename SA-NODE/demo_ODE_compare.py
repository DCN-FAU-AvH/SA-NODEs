...
from models.plot import plot_ODEs_compare, plot_ODEs_compare_error
...
def main():
    ...
    datasets = ODE_Dataset(ode_type, step=0.2)
    t, u = datasets[:]
    ...
    # TEST
    # =============================================================================
    datasets = ODE_Dataset(ode_type, step=0.5)
    t, u = datasets[:]
    # separate the data into u_train and u_test
    train_size = int(0.5 * datasets.length_u())
    shuffled_indices = torch.randperm(datasets.length_u())
    ...
    # plot the results
    plot_ODEs_compare(u_real, u_train_trad, u_test_trad, u_train_semi, u_test_semi, ode_type)
    ...
    # print the max and min error in 3 decimal places as scientific notation
    print('Traditional neural ODE model:')
    print('Max error:', '{:.2e}'.format(error_trad_max))
    print('End error:', '{:.2e}'.format(error_trad_end))
    print('SA-ODE model:')
    print('Max error:', '{:.2e}'.format(error_semi_max))
    print('End error:', '{:.2e}'.format(error_semi_end))

    plot_ODEs_compare_error(t, error_trad, error_semi)
