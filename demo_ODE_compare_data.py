...
from models.plot import plot_data_error
...
def main():
    ...
    datasets = ODE_Dataset(ode_type, step=step)
    t, u = datasets[:]
    ...
    return error_trad_max, error_semi_max


if __name__ == '__main__':
    step_list = [1, 0.5, 0.4, 0.2, 0.1]
    error_trad = []
    error_semi = []
    for num_epochs in [10000]:
        for hidden_dim in [1000]:
            for step in step_list:
                error_trad_max, error_semi_max = main()
                error_trad.append(error_trad_max)
                error_semi.append(error_semi_max)
    
    # plot the results
    data_list = ((4/np.array(step_list) + 1)**2 * 0.5)
    data_list = [int(i) for i in data_list]
    plot_data_error(data_list, error_semi, error_trad)
